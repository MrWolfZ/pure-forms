import { Boxed, unbox, ValidationErrors } from 'pure-forms';

// this regex is taken from the @angular/forms source code
// tslint:disable-next-line:max-line-length
export const PURE_FORMS_EMAIL_VALIDATION_REGEXP = /^(?=.{1,254}$)(?=.{1,64}@)[-!#$%&'*+/0-9=?A-Z^_`a-z{|}~]+(\.[-!#$%&'*+/0-9=?A-Z^_`a-z{|}~]+)*@[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?(\.[A-Za-z0-9]([A-Za-z0-9-]{0,61}[A-Za-z0-9])?)*$/;
...
// @ts-ignore
declare module 'pure-forms/src/state' {
...
export function email<T extends string | Boxed<string> | null | undefined>(value: T): ValidationErrors {
  ...
  if (PURE_FORMS_EMAIL_VALIDATION_REGEXP.test(value as string)) {
    return {};
  }

  return {
    email: {
      pattern: PURE_FORMS_EMAIL_VALIDATION_REGEXP.toString(),
      actual: value,
    },
  };
}
