...
const DIST_DIR = `dist`;
const MODULES_DIR = `${DIST_DIR}/modules`;
...
const PACKAGES = [
  {
    ...
    dir: '.',
    modulesDir: '.',
    bundleFileName: 'pure-forms',
    moduleFileName: 'pure-forms',
    moduleName: 'pure-forms',
  },
  {
    name: 'validation',
    dir: './validation',
    modulesDir: '.',
    bundleFileName: 'pure-forms-validation',
    moduleFileName: 'pure-forms-validation',
    moduleName: 'pure-forms-validation',
  },
];
...
for (var pkg of PACKAGES) {
  ...
  shell.echo(`Starting tsc compilation...`);
  if (shell.exec(`tsc -p ${pkg.dir}/tsconfig-build.json`).code !== 0) {
    shell.echo(chalk.red(`Error: tsc compilation failed!`));
    shell.exit(1);
  }

  shell.echo(chalk.green(`tsc compilation completed!`));
  ...
  if (shell.exec(`rollup -f es -n ${pkg.moduleName} -i ${DIST_DIR}/${pkg.dir}/index.js -o ${MODULES_DIR}/${pkg.modulesDir}/${pkg.moduleFileName}.js --sourcemap -e ${rollupConfig.external.join(',')}`).code !== 0) {
  ...
  if (shell.exec(`node scripts/map-sources -f ${MODULES_DIR}/${pkg.modulesDir}/${pkg.moduleFileName}.js`).code !== 0) {
    shell.echo(chalk.red(`Error: Bundling failed while mapping sources!`));
    shell.exit(1);
  }
